...
class BruteForceSurface():
    def __init__(self, composition, adsorbates, ads_atoms, sites, coordinates, height,
        ...
        # Metal parameters
        # Caution: metals are sorted, which will affect the order of element identity
        ...
        #list of all meatl_idx
        metal_ids = list(it.chain.from_iterable([[metal_idx] * int(n) for metal_idx, n in n_each_metal.items()]))
        #shuffle all metal_idx
        #WX making calculation identical and don't affact lhs sampling
        #print("NO.Atoms:", len(metal_ids))
        rng = random.Random(0)
        rng.shuffle(metal_ids)
        ...
        while True:
            ...
            for key in self.grid_dict_net.keys():
                ...
                if np.min(self.grid_dict_net[key]) < min_e:
                    min_e, min_e_key = self.grid_dict_net[key].min(), key #wx find min binding energy and it's adsorbte type

...
def _local_optimization(x, y, f, eval_func, bounds, constr_func, delta_s, n_constr, transformation=None):
    '''
    ...
    #wx transform x from stochastic sampling 
    x = transformation.do(x)
    x = transformation.undo(x)
    ...
    if constr_func is not None:
        ...
        con = lambda x: 1-sum(x)
        delta = 1e-5
        #keep_feasible = False means initial guess can violate constraints
        wx_constraints = NonlinearConstraint(con, 0-delta, 0.9)
    ...
    #wx, gradient_respecting_constraints       
        def gradient_respecting_constraints(eval_func, constraints=[0, 0.9], eps=1e-8):
            def jac(x):
                fx, dfx = eval_func(x, return_values_of=['F', 'dF'])
                grad = ((fx - z) / np.linalg.norm(fx - z)) @ dfx
                if constraints[0] - eps <= 1-sum(x) <= constraints[1] + eps:
                    return grad
                else:
                    return -grad
            return jac
    ...
    #WX, use SLSQP
         res = minimize(fun, x, method='SLSQP', jac=gradient_respecting_constraints(eval_func), bounds=np.array(bounds).T, constraints=wx_constraints, tol=1e-4)
...
def _first_order_approximation(x_opt, directions, bounds, constr_func, n_grid_sample, n_constr):
    '''
    ...
    if direction_dim > n_obj - 1:
        ...
        print("first order less then d-1")
        return x_samples
    ...
    while len(x_samples) < n_grid_sample:
        ...
        if constr_func is not None:
            #WX: impelement more constraints
            if n_constr > 1:
...
def _pareto_discover(xs, eval_func, bounds, constr_func, delta_s, origin, origin_constant, n_grid_sample, queue, n_constr, transformation=None):
    '''
    ...
    for i, (x, y, f) in enumerate(zip(xs, ys, fs)):
        ...
        x_opt = _local_optimization(x, y, f, eval_func, bounds, constr_func, delta_s, n_constr, transformation=transformation)

        directions = _get_optimization_directions(x_opt, eval_func, bounds)

       # get new valid samples from local manifold
        x_samples = _first_order_approximation(x_opt, directions, bounds, constr_func, n_grid_sample, n_constr)
...
class ParetoDiscoveryAlgorithm(Algorithm):
    ...
    def __init__(self,
        ...
        # print("delta_p:", delta_p, "delta_s:", delta_s, "delta_b:", delta_b, "label_cost:", label_cost, "n_grid_sample:", n_grid_sample, "cells:", n_cell, "ref-point: TODO") 
    ...
    def _initialize(self):
        ...
        if pop_constr is not None:
            ...
            #WX for more than one constraint
            if self.problem.n_constr > 1:
                pop = pop[np.all(pop_constr <= 0, axis=1)]
            else:
            ...
            while len(pop) < self.pop_size:
                ...
                #WX for more than one constraint
                if self.problem.n_constr > 1:
                    new_pop = new_pop[np.all(self.problem.evaluate_constraint(new_pop_x) <= 0, axis=1)]
                else:
    ...
    def _next(self):
            ...
            if len(x) > 0:
                p = Process(target=_pareto_discover, 
                    args=(x, self.problem.evaluate, [self.problem.xl, self.problem.xu], self.constr_func, self.delta_s, 
                        self.buffer.origin, self.buffer.origin_constant, self.n_grid_sample, queue, self.problem.n_constr, self.problem.transformation))
                p.start()
                process_count += 1
        ...
        x_samples_all = np.vstack(x_samples_all) #5700+
        y_samples_all = self.problem.evaluate(x_samples_all, return_values_of=['F'])
        #wx: check quality of surrogate model and stochastic sampling
    ...
    def _stochastic_sampling(self):
        ...
        while xs_final.shape[0] < num_target:
            ...
            # create unit vector of each row.
            ...
            # wx: relative scaling
            #xs_perturb = xs + 1.0 / (2 ** delta) * d * xs
            ...
            if self.constr_func is None:
                ...
                flag = constr <= 0
                #WX for more than one constraints.
                if self.problem.n_constr > 1:
                    xs_final = np.vstack((xs_final, xs_perturb[np.all(flag, axis=1)]))
    ...
    def propose_next_batch(self, curr_pfront, ref_point, batch_size):
        ...
        if len(approx_x) >= batch_size:
            ...
            print("approximation result is not enough")
...
class ParetoDiscovery(Solver):
    ...
    '''
    def __init__(self, problem, n_gen=10, pop_size=100, n_process=cpu_count(), **kwargs): 
        super().__init__(problem)
        self.n_gen = n_gen
        #wx: test n_process = 1 for debugging
        self.algo = ParetoDiscoveryAlgorithm(pop_size=pop_size, n_process=n_process, **kwargs)

    def _solve(self, X, Y, batch_size):
        #WX sample more point, shouldn't we also consider history?
        #X = np.vstack([X, lhs(X.shape[1], 20)])
        ...
        #print("ref_point:", ref_point ,"origin", algo.buffer.origin) #algo.fam_lbls
